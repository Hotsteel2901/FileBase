...
| `dist/filebase-v2.0.0.zip` | Magisk / KernelSU / APatch 卡刷模块（含 WebUI 面板 + 日志管理） |
...
├── META-INF/                  卡刷兼容
├── build.sh                   一键构建 zip
└── filebase-v2.0.0.zip        已构建的刷入包

dist/                          发布归档
├── webserver.tar.gz           独立版
└── filebase-v2.0.0.zip        卡刷模块
```
...
1. 在网页端**连续快速点击顶部 "FILEBASE" 品牌标识 10 次**（每次间隔需在 0.5 秒以内）
2. 连续 10 次点击后，右上角控制区会出现**登录按钮**
3. 关闭登录弹窗后该按钮**不会消失**——直到手动刷新页面才会重新隐藏
4. 刷新页面后需再次执行 10 次快速点击才能重新显示
...
- 10 次点击激活仅是 UI 层面的隐藏机制，不是安全边界——管理员 API 端点始终可用，只是默认不在界面中显示
